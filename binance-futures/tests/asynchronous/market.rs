...
mod futures_market_api_integration_tests {
    ...
    use binance_futures::asynchronous::market::MarketApi;
    ...
    pub fn new_test_client() -> MarketApi<'static, HmacSha256<'static>> {
        MarketApi::new(Client::new(
            &Host::Api,
    ...
    async fn test_get_server_time() {
        ...
        assert!(market_api.get_server_time().await.unwrap().server_time > 0);
    }
    ...
    async fn test_get_exchange_info() {
        ...
        let exchange_info = market_api.get_exchange_info(&params).await.unwrap();
    ...
    async fn test_get_depth() {
        ...
        let depth: DepthResponse = market_api.get_depth(&params).await.unwrap();
    ...
    async fn test_get_trades() {
        ...
        let trades: Vec<TradesResponse> = market_api.get_trades(&params).await.unwrap();
    ...
    async fn test_get_historical_trades() {
        ...
        let historical_trades: Vec<HistoricalTradesResponse> =
            market_api.get_historical_trades(&params).await.unwrap();
    ...
    async fn test_get_klines() {
        ...
        let klines: Vec<KlinesResponse> = market_api.get_klines(&params).await.unwrap();
    ...
    async fn test_get_mark_price() {
        ...
        let mark_price: MarkPriceResponse = market_api.get_mark_price(&params).await.unwrap();
    ...
    #[tokio::test]
    async fn test_get_mark_price_list() {
        let market_api: MarketApi<HmacSha256> = new_test_client();

        let mark_price_list: Vec<MarkPriceResponse> =
            market_api.get_mark_price_list().await.unwrap();
    ...
    async fn test_get_funding_rate_history() {
        ...
        let funding_rate_history: Vec<FundingRateHistoryResponse> =
            market_api.get_funding_rate_history(&params).await.unwrap();
    ...
    #[tokio::test]
    async fn test_get_ticker24() {
        let market_api: MarketApi<HmacSha256> = new_test_client();

        let params: Symbol = Symbol::new("BTCUSDT");

        let ticker24h: Ticker24hResponse = market_api.get_ticker24h(&params).await.unwrap();
    ...
    #[tokio::test]
    async fn test_get_ticker24_list() {
        let market_api: MarketApi<HmacSha256> = new_test_client();

        let ticker24h_list: Vec<Ticker24hResponse> = market_api.get_ticker24h_list().await.unwrap();
    ...
    async fn test_get_ticker_price() {
        ...
        let ticker24h: TickerPriceResponse = market_api.get_ticker_price(&params).await.unwrap();
    ...
    #[tokio::test]
    async fn test_get_ticker_price_list() {
        let market_api: MarketApi<HmacSha256> = new_test_client();

        let ticker24h_list: Vec<TickerPriceResponse> =
            market_api.get_ticker_price_list().await.unwrap();

        assert!(ticker24h_list.len() > 0);
    }

    #[tokio::test]
    async fn test_get_ticker_price_v2() {
        let market_api: MarketApi<HmacSha256> = new_test_client();

        let params: Symbol = Symbol::new("BTCUSDT");

        let ticker24h: TickerPriceResponse = market_api.get_ticker_price_v2(&params).await.unwrap();
    ...
    #[tokio::test]
    async fn test_get_ticker_price_v2_list() {
        let market_api: MarketApi<HmacSha256> = new_test_client();

        let ticker24h_list: Vec<TickerPriceResponse> =
            market_api.get_ticker_price_v2_list().await.unwrap();
    ...
    async fn test_get_book_ticker() {
        ...
        let book_ticker = market_api.get_book_ticker(&params).await.unwrap();
    ...
    #[tokio::test]
    async fn test_get_book_ticker_list() {
        let market_api: MarketApi<HmacSha256> = new_test_client();

        let book_ticker: Vec<BookTickerResponse> = market_api.get_book_ticker_list().await.unwrap();

        assert!(book_ticker.len() > 0);
    }

    #[tokio::test]
    async fn test_get_delivery_price_list() {
        let market_api: MarketApi<HmacSha256> = new_test_client();
    ...
    #[tokio::test]
    async fn test_get_open_interest_history_list() {
        let market_api: MarketApi<HmacSha256> = new_test_client();

        let params: OpenInterestHistoryParams =
            OpenInterestHistoryParams::new("SOLUSDT", &Interval::Minutes5);

        let open_interet_history: Vec<OpenInterestHistoryResponse> = market_api
            .get_open_interest_history_list(&params)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn test_get_top_long_position_ratio_list() {
        let market_api: MarketApi<HmacSha256> = new_test_client();
    ...
    #[tokio::test]
    async fn test_get_top_long_account_ratio_list() {
        let market_api: MarketApi<HmacSha256> = new_test_client();

        let params: TopLongShortAccountRatioParams =
            TopLongShortAccountRatioParams::new("SOLUSDT", &Interval::Minutes5);

        let data: Vec<TopLongShortAccountRatioResponse> = market_api
            .get_top_long_account_ratio_list(&params)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn test_get_global_long_account_ratio_list() {
        let market_api: MarketApi<HmacSha256> = new_test_client();
    ...
    #[tokio::test]
    async fn test_get_taker_buy_sell_volume_list() {
        let market_api: MarketApi<HmacSha256> = new_test_client();

        let params: TakerBuySellVolumeParams =
            TakerBuySellVolumeParams::new("SOLUSDT", &Interval::Minutes5);

        let data: Vec<TakerBuySellVolumeResponse> = market_api
            .get_taker_buy_sell_volume_list(&params)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn test_get_basis_list() {
        let market_api: MarketApi<HmacSha256> = new_test_client();

        let params: BasisParams =
            BasisParams::new("SOLUSDT", ContractType::Perpetual, &Interval::Minutes5, 17);

        let data: Vec<BasisResponse> = market_api.get_basis_list(&params).await.unwrap();
