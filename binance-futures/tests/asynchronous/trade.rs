...
mod futures_trade_api_integration_test {
    ...
    async fn test_new_limit_test_order() {
        ...
        let new_order: TestOrderResponse = trade_api.send_new_test_order(&params).await.unwrap();
    ...
    #[tokio::test]
    async fn test_new_stop_loss_test_order() {
        ...
        let new_order_error: BinanceError =
            trade_api.send_new_test_order(&params).await.unwrap_err();
    ...
    #[tokio::test]
    async fn test_send_set_leverage() {
        let trade_api = shared_test_trade();

        let params: SetLeverageParams = SetLeverageParams::new("ETHUSDT", 2);

        let eth_leverage: SetLeverageResponse = trade_api.send_set_leverage(&params).await.unwrap();
    ...
    async fn test_get_position_risk() {
        ...
        let positions: Vec<PositionRiskV3Response> =
            trade_api.get_position_risk_v3(&params).await.unwrap();
    ...
    async fn test_send_new_order() {
        ...
        let current_order: GetOrderResponse = trade_api.get_order(&params2).await.unwrap();
